...
data_frame = pd.DataFrame(columns=['Emotion', 'Emotion intensity', 'Gender','Path'])
...
# Plot the distribution of emotion intensities in the dataset
# fig = plt.figure()
# ax = fig.add_subplot(111)
...
X_train = np.reshape(X_train, shape=(b,c,h,w))

b,c,h,w = X_test.shape
X_test = np.reshape(X_test, shape=(b,-1))
X_test = scaler.transform(X_test)
X_test = np.reshape(X_test, shape=(b,c,h,w))

b,c,h,w = X_val.shape
X_val = np.reshape(X_val, shape=(b,-1))
X_val = scaler.transform(X_val)
X_val = np.reshape(X_val, shape=(b,c,h,w))
...
torch.save(model.state_dict(),os.path.join(SAVE_PATH,'cnn_transf_parallel_model_1.pt'))
print('Model is saved to {}'.format(os.path.join(SAVE_PATH,'cnn_transf_parallel_model_1.pt')))
...
log_df.to_csv("logs/parallel_cnn_transformer.csv", index=False)