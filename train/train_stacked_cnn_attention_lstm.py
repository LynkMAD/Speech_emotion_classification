...
DATA_PATH = 'D:/speech2text/speech_emotion_classification/data/audio_speech_actors_01-24'
SAMPLE_RATE = 48000
new_data = []
...
for dirname, _, filenames in os.walk(DATA_PATH):
    for filename in filenames:
        ...
        new_data.append({"Emotion": emotion,
...
X_train = np.reshape(X_train, shape=(b,-1))
X_train = scaler.fit_transform(X_train)
X_train = np.reshape(X_train, shape=(b,t,c,h,w))

b,t,c,h,w = X_test.shape
X_test = np.reshape(X_test, shape=(b,-1))
X_test = scaler.transform(X_test)
X_test = np.reshape(X_test, shape=(b,t,c,h,w))

b,t,c,h,w = X_val.shape
X_val = np.reshape(X_val, shape=(b,-1))
X_val = scaler.transform(X_val)
X_val = np.reshape(X_val, shape=(b,t,c,h,w))
...
torch.save(model.state_dict(),os.path.join(SAVE_PATH,'cnn_attention_lstm_model_1.pt'))
print('Model is saved to {}'.format(os.path.join(SAVE_PATH,'cnn_attention_lstm_model_1.pt')))

LOAD_PATH = os.path.join(os.getcwd(),'models')
model = HybridModel(len(EMOTIONS))
model.load_state_dict(torch.load(os.path.join(LOAD_PATH,'cnn_attention_lstm_model_1.pt')))
print('Model is loaded from {}'.format(os.path.join(LOAD_PATH,'cnn_attention_lstm_model_1.pt')))
...
log_df.to_csv("logs/stacked_cnn_attention_lstm.csv", index=False)